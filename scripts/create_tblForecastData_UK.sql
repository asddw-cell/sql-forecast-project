...
Create transactional ForecastData table for UK

==============================================================================

Script Purpose:
	This script creates a system versioned table called 'tblForecastData_UK' after checking if it already exists.
	If the table exists, it is dropped and recreated. Also creates primary and foreign keys.

Warning:
	Running this script will drop the entire 'tblForecastData_UK' table if it exists.
...
/****** Object:  Table [dbo].[tblForecastData_UK]    Script Date: 06/02/2026 14:13:14 ******/
SET ANSI_NULLS ON
GO
...
IF OBJECT_ID('[dbo].[tblForecastData_UK]', 'U') IS NOT NULL
BEGIN
	-- Disable system versioning first
	ALTER TABLE [dbo].[tblForecastData_UK]
		SET (SYSTEM_VERSIONING = OFF);

	DROP TABLE [dbo].[tblForecastData_UK];
	DROP TABLE [dbo].[tblForecastData_UKHistory];
END
GO

-- Create the transactional Forecast table for UK
CREATE TABLE [dbo].[tblForecastData_UK] (
	...
	,CONSTRAINT FK_tblForecastData_UK_ForecastType FOREIGN KEY ([ForecastType]) 
		REFERENCES [dbo].[tblForecastType]([Code])
	-- Add foreign key constraint for SalesChannel
	,CONSTRAINT FK_tblForecastData_UK_SalesChannel FOREIGN KEY ([SalesChannel]) 
	...
	,CONSTRAINT FK_tblForecastData_UK_ItemNo FOREIGN KEY ([ItemNo]) 
		REFERENCES [dbo].[tblItem_UK]([ItemNo])
	-- Add foreign key constraint for PriceType
	,CONSTRAINT FK_tblForecastData_UK_PriceType FOREIGN KEY ([PriceType]) 
		REFERENCES [dbo].[tblPriceType]([Code])
	,PERIOD FOR SYSTEM_TIME([SysStartTime], [SysEndTime])
	) ON [PRIMARY]
	-- Add history table for recording changes
	WITH (SYSTEM_VERSIONING = ON (HISTORY_TABLE = [dbo].[tblForecastData_UKHistory]))
GO

-- Define default versioning start and end dates for rows
ALTER TABLE [dbo].[tblForecastData_UK] ADD DEFAULT(getutcdate())
FOR [SysStartTime]
GO

ALTER TABLE [dbo].[tblForecastData_UK] ADD DEFAULT(CONVERT([datetime2], '9999 - 12 - 31 23 : 59 : 59.9999999'))
FOR [SysEndTime]
GO
